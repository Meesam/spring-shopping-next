...
export const updateCategory = async (categoryRequest: CategoryRequest) => {
    const response = await serverFetch('/category/update', { method: 'POST', body: JSON.stringify(categoryRequest) });
    return response.json();
};
